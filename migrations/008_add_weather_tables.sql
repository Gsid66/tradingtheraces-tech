-- Migration 008: Add Weather Tables
-- Purpose: Store weather data for race tracks from MET Norway API

-- Create track_weather table for hourly forecasts
CREATE TABLE IF NOT EXISTS track_weather (
  id SERIAL PRIMARY KEY,
  track_name VARCHAR(100) NOT NULL,
  latitude DECIMAL(10, 7) NOT NULL,
  longitude DECIMAL(10, 7) NOT NULL,
  forecast_time TIMESTAMP WITH TIME ZONE NOT NULL,
  temperature DECIMAL(5, 2),
  wind_speed DECIMAL(5, 2),
  wind_direction NUMERIC(5, 2),
  precipitation DECIMAL(5, 2),
  weather_symbol VARCHAR(50),
  weather_description TEXT,
  fetched_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  
  -- Ensure no duplicate entries for same track/time
  UNIQUE(track_name, forecast_time)
);

-- Add indexes for quick lookups
CREATE INDEX IF NOT EXISTS idx_track_weather_track_time ON track_weather(track_name, forecast_time);
CREATE INDEX IF NOT EXISTS idx_track_weather_fetched ON track_weather(fetched_at);
CREATE INDEX IF NOT EXISTS idx_track_weather_forecast_time ON track_weather(forecast_time);

-- Add weather columns to pf_meetings table
ALTER TABLE pf_meetings 
  ADD COLUMN IF NOT EXISTS current_temperature DECIMAL(5, 2),
  ADD COLUMN IF NOT EXISTS current_wind_speed DECIMAL(5, 2),
  ADD COLUMN IF NOT EXISTS current_wind_direction NUMERIC(5, 2),
  ADD COLUMN IF NOT EXISTS current_weather_symbol VARCHAR(50),
  ADD COLUMN IF NOT EXISTS weather_updated_at TIMESTAMP WITH TIME ZONE;

-- Add comment for track_weather table
COMMENT ON TABLE track_weather IS 'Hourly weather forecasts for race tracks from MET Norway API';

-- Add comments for columns
COMMENT ON COLUMN track_weather.track_name IS 'Normalized track name (lowercase)';
COMMENT ON COLUMN track_weather.latitude IS 'Track latitude coordinate';
COMMENT ON COLUMN track_weather.longitude IS 'Track longitude coordinate';
COMMENT ON COLUMN track_weather.forecast_time IS 'Time this forecast is for (in track local time)';
COMMENT ON COLUMN track_weather.temperature IS 'Temperature in degrees Celsius';
COMMENT ON COLUMN track_weather.wind_speed IS 'Wind speed in m/s';
COMMENT ON COLUMN track_weather.wind_direction IS 'Wind direction in degrees (0-360)';
COMMENT ON COLUMN track_weather.precipitation IS 'Precipitation amount in mm';
COMMENT ON COLUMN track_weather.weather_symbol IS 'MET Norway weather symbol code';
COMMENT ON COLUMN track_weather.weather_description IS 'Human-readable weather description';
COMMENT ON COLUMN track_weather.fetched_at IS 'When this forecast was fetched from MET Norway API';

-- Add comments for pf_meetings weather columns
COMMENT ON COLUMN pf_meetings.current_temperature IS 'Current temperature at track in degrees Celsius';
COMMENT ON COLUMN pf_meetings.current_wind_speed IS 'Current wind speed at track in m/s';
COMMENT ON COLUMN pf_meetings.current_wind_direction IS 'Current wind direction at track in degrees (0-360)';
COMMENT ON COLUMN pf_meetings.current_weather_symbol IS 'Current MET Norway weather symbol code';
COMMENT ON COLUMN pf_meetings.weather_updated_at IS 'When weather was last updated for this meeting';

-- ============================================================================
-- ENHANCED WEATHER INTEGRATION: Historical Data and Analysis
-- ============================================================================

-- Historical weather observations (permanent storage)
CREATE TABLE IF NOT EXISTS track_weather_history (
  id SERIAL PRIMARY KEY,
  track_name VARCHAR(100) NOT NULL,
  meeting_id VARCHAR(100),  -- Link to pf_meetings
  observation_time TIMESTAMP WITH TIME ZONE NOT NULL,
  temperature DECIMAL(5, 2),
  feels_like_temperature DECIMAL(5, 2),
  wind_speed DECIMAL(5, 2),
  wind_gust DECIMAL(5, 2),
  wind_direction NUMERIC(5, 2),
  wind_direction_compass VARCHAR(3),
  humidity INTEGER,
  precipitation DECIMAL(5, 2),
  precipitation_probability INTEGER,
  weather_symbol VARCHAR(50),
  weather_description TEXT,
  visibility INTEGER,
  pressure DECIMAL(7, 2),
  cloud_cover INTEGER,
  uv_index INTEGER,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Weather conditions at exact race time (CRITICAL FOR ANALYSIS)
CREATE TABLE IF NOT EXISTS race_weather_conditions (
  id SERIAL PRIMARY KEY,
  race_id VARCHAR(100) NOT NULL,  -- Links to pf_races
  meeting_id VARCHAR(100) NOT NULL,
  track_name VARCHAR(100) NOT NULL,
  race_number INTEGER NOT NULL,
  race_start_time TIMESTAMP WITH TIME ZONE NOT NULL,
  
  -- Actual conditions AT race time
  temperature DECIMAL(5, 2),
  feels_like DECIMAL(5, 2),
  wind_speed DECIMAL(5, 2),
  wind_gust DECIMAL(5, 2),
  wind_direction NUMERIC(5, 2),
  wind_direction_compass VARCHAR(3),
  humidity INTEGER,
  precipitation_last_hour DECIMAL(5, 2),
  precipitation_probability INTEGER,
  weather_symbol VARCHAR(50),
  weather_condition VARCHAR(100),
  visibility INTEGER,
  pressure DECIMAL(7, 2),
  cloud_cover INTEGER,
  
  -- Track impact indicators
  track_bias_direction VARCHAR(50),  -- 'favoring_inside', 'favoring_outside', 'neutral'
  wind_impact_category VARCHAR(20),  -- 'light', 'moderate', 'strong', 'severe'
  weather_impact_score INTEGER,  -- 1-10 scale
  conditions_note TEXT,
  
  recorded_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(race_id)
);

-- Indexes for fast analysis queries
CREATE INDEX IF NOT EXISTS idx_weather_history_track_time ON track_weather_history(track_name, observation_time);
CREATE INDEX IF NOT EXISTS idx_weather_history_meeting ON track_weather_history(meeting_id);
CREATE INDEX IF NOT EXISTS idx_weather_history_observation ON track_weather_history(observation_time DESC);

CREATE INDEX IF NOT EXISTS idx_race_weather_race ON race_weather_conditions(race_id);
CREATE INDEX IF NOT EXISTS idx_race_weather_track_time ON race_weather_conditions(track_name, race_start_time);
CREATE INDEX IF NOT EXISTS idx_race_weather_meeting ON race_weather_conditions(meeting_id);
CREATE INDEX IF NOT EXISTS idx_race_weather_conditions ON race_weather_conditions(weather_condition);

-- Add comments for new tables
COMMENT ON TABLE track_weather_history IS 'Historical weather observations for permanent storage and analysis';
COMMENT ON TABLE race_weather_conditions IS 'Weather conditions at exact race start time for performance analysis';

COMMENT ON COLUMN track_weather_history.meeting_id IS 'Optional link to pf_meetings table';
COMMENT ON COLUMN track_weather_history.observation_time IS 'Time of weather observation';
COMMENT ON COLUMN track_weather_history.feels_like_temperature IS 'Apparent temperature accounting for wind chill';
COMMENT ON COLUMN track_weather_history.wind_gust IS 'Maximum wind gust speed in m/s';

COMMENT ON COLUMN race_weather_conditions.race_id IS 'Unique race identifier linking to pf_races';
COMMENT ON COLUMN race_weather_conditions.track_bias_direction IS 'Track bias caused by weather conditions';
COMMENT ON COLUMN race_weather_conditions.wind_impact_category IS 'Severity of wind impact on race';
COMMENT ON COLUMN race_weather_conditions.weather_impact_score IS 'Overall weather impact score (1=minimal, 10=severe)';
