#!/usr/bin/env node
/**
 * Scrape and import daily results from racing-bet-data.com
 * Usage: npx tsx scripts/uk-racing/scrape-daily-results.ts [--date=YYYY-MM-DD]
 */
...
async function main() {
  ...
  console.log('🚀 UK/IRE Racing Daily Results Scraper');
  ...
  try {
    // Create scraper log
    logId = await createScraperLog(pool, 'results', scrapeDate);
    
    // Scrape files
    console.log('🌐 Scraping racing-bet-data.com...');
    const downloadResult = await scrapeFiles('results', scrapeDate);
    
    if (!downloadResult.success) {
      console.error(`❌ Failed to download results: ${downloadResult.error}\n`);
      ...
      console.log('   npx tsx scripts/uk-racing/import-csv.ts --file=<path> --type=results\n');
