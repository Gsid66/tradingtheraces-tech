...
import type { PFMeeting } from '@/lib/integrations/punting-form/types';  // ← Changed from /client to /types
...
function MeetingCard({ meeting, index }: { meeting: PFMeeting; index: number }) {
  ...
  console.log(`  -> Generated ${raceNumbers.length} race numbers:`, raceNumbers);
    ...
    >
      ...
      <div className="relative p-6 bg-gradient-to-r from-white/10 to-transparent border-b border-white/10">
        <div className="flex items-start justify-between flex-wrap gap-4">
          ...
          {/* Race Count Badge - Show even if 0 for debugging */}
          ...
          <div className="text-center py-8">
            <div className="inline-flex items-center gap-3 px-6 py-4 bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10">
              ...
              <span className="text-sm font-medium text-white/60">
                Race information not available (races: {meeting.races === null ? 'null' : meeting.races})
              </span>
            </div>
          </div>
        )}
