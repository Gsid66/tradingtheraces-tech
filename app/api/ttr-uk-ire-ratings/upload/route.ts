...
import { parseTTRCSV, TTRRating } from '@/lib/parsers/ttr-uk-ire-csv-parser';
 ...
 */
async function importRatings(pool: Pool, ratings: TTRRating[]): Promise<ImportResult> {
  ...
  try {
    ...
    for (let i = 0; i < ratings.length; i += BATCH_SIZE) {
      ...
      const query = `
        INSERT INTO ttr_uk_ire_ratings (
          race_date, track_name, race_name, race_number, saddle_cloth,
          horse_name, jockey_name, trainer_name, rating, price
        )
        VALUES ${placeholders.join(', ')}
        ON CONFLICT (race_date, track_name, race_number, horse_name)
        DO UPDATE SET
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Parse CSV
    let ratings: TTRRating[];
    try {
      ratings = parseTTRCSV(csvContent);
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown error';
      return NextResponse.json(
    ...
    const message = error instanceof Error ? error.message : 'Unknown error';
    console.error('Error in TTR UK/IRE ratings upload:', error);
