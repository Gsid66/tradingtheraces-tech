...
async function getRatingsForDate(date: string): Promise<TTRRatingData[]> {
  ...
  try {
    ...
    const query = `
      SELECT 
        race_date, track_name, race_name, race_number,
...
function convertToCSV(ratings: TTRRatingData[]): string {
  ...
  const rows = ratings.map(rating => [
    ...
    rating.track_name,
    rating.race_name,
    rating.race_number.toString(),
    rating.saddle_cloth !== null ? rating.saddle_cloth.toString() : '',
    rating.horse_name,
    rating.jockey_name || '',
    rating.trainer_name || '',
    rating.rating !== null ? rating.rating.toString() : '',
    rating.price !== null ? rating.price.toString() : ''
  ]);
  ...
  try {
    ...
    const response = new NextResponse(csvContent, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="ttr-ratings-${date}.csv"`,
      }
    });

    const executionTime = Date.now() - startTime;
    console.log(`✅ Downloaded ${ratings.length} ratings for ${date} in ${executionTime}ms`);

    return response;
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown error';
    console.error('Error in TTR UK/IRE ratings download:', error);
